...
/*!
    ...
    The IntentsInterface is not meant to be used as is in the system, instead it is acting as the receiver of the requests
    coming from the applications using the Qt.openUrlExternally function as shown in the example below.
    ...
    \qmlproperty string activeAppId
    Holds the active application id. It is used to store the active application id before an intent request is submitted in
    order to be able to navigate back to it accordingly.

    \qmlproperty var history
    Holds the history of activities started and it's used for a sequential navigation back to the initial state

    \qmlmethod sendRequest(id, params)
    Requests the action to be taken in the UI. It uses the Application Managers' acknowledgeOpenUrlRequest
    function to open the requested application passing the given url to it. It also keeps and updates the history with
    all the steps performed each time the function is called.
